...
- **A video is made in scenes.** A kind whose works are made in scenes — a video, a short — carries a storyboard: one row per scene with its number, the part of the text it plays against, its seconds, a kind of shot from your profile's list and a prompt block per generator, each with its own copy button. The scaffold is fixed; you type inside the blocks. What every scene shares — the hero, the palette, the lens — is a body with revisions like the plot. See [Scenes](https://lacodda.github.io/kilna/guides/scenes/).
